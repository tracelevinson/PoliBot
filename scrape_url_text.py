...
# Save pandas-friendly version for creating article embeddings in build_starspace.py
articles_df[['url','source','text']].to_csv(full_detailed_pandas_path, index=False)
...
# Tab-separate sentences and concatenate columns for efficient TSV importation
articles_df['text'] = (articles_df.text.str.replace(' \. ',' .\t')
                                       .str.replace(' \? ',' ?\t')
                                       .str.replace(' \! ',' !\t'))

articles_df['concat'] = articles_df[['url','source','text']].apply(lambda x: '\t'.join(x), axis=1)

# Save clean, tab-separated articles dataset for Starspace model
articles_df.concat.to_csv(full_detailed_path, sep='\t', header=False, index=False)
